...
import type { ParsedRow } from '@/core/import';
...
function dedupeKey(accountId: string, date: string, amount: number, description: string): string {
...
export async function importTransactions(
  ...
  for (const r of rows) {
    const key = dedupeKey(accountId, r.date, r.amount, r.description);
  ...
  await db
    ...
    .set({ importedCount: toInsert.length, duplicateCount: duplicates })
    .where(eq(importBatches.id, batch.id));

  const transfersDetected = await runTransferDetection(userId);
  return { imported: toInsert.length, duplicates, batchId: batch.id, transfersDetected };
}
